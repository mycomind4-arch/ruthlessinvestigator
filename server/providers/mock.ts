...
const GAP_ANALYSIS_RESPONSE = JSON.stringify({
  gaps: [
    { question: "What is the actual utilization rate of recently completed data centers?", importance: "CRITICAL", expected_impact: "Would distinguish demand-driven construction from speculative overbuilding" },
    { question: "What proportion of new capacity is contracted for AI workloads vs general cloud?", importance: "HIGH", expected_impact: "Would determine whether AI demand is truly the primary driver" },
    { question: "What is the completion rate of announced data-center projects?", importance: "HIGH", expected_impact: "Would clarify whether announced capacity reflects real construction" },
    { question: "What role do tax incentives play in data-center location decisions?", importance: "MODERATE", expected_impact: "Would assess whether financial incentives drive construction independent of demand" },
    { question: "Are data-center companies building speculatively without committed tenants?", importance: "HIGH", expected_impact: "Would distinguish demand-driven from speculative construction" }
  ]
}, null, 2);
...
function getResponse(req: AIRequest): string {
  ...
  // ── Skeptic (but check for gap analysis prompt first) ──────────
  if (/skeptic/i.test(sp)) {
    if (/gap|information.*gap|missing.*evidence/i.test(prompt)) return GAP_ANALYSIS_RESPONSE;
