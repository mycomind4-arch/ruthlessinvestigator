...
function makeSteps(observation: LearningObservation): SkillStep[] {
  ...
  if (steps.length === 0) {
    steps.push({
      id: "learned-step-1",
      type: "ANALYZE_CLAIM",
      description: `Investigate the recurring problem: ${observation.problem}`,
      inputs: ["question"],
      outputs: ["finding"],
      expectedOutput: "A traceable finding that addresses the capability gap.",
    });
  }
...
/**
 * Persistent-in-process learner. The registry remains the source of truth;
 * this class only accumulates observations and emits proposals.
 */
export class SkillLearningEngine {
  ...
  observe(observation: LearningObservation): SkillLearningDecision {
    ...
    // If an existing active skill already addresses the problem and execution
    // succeeded, learning is useful only as performance telemetry.
    const existing = this.registry.searchSkills({ name: observation.problem, status: "ACTIVE" });
    if (existing.length > 0 && observation.success) {
      return { action: "NO_ACTION", reason: "An active skill already addresses this recurring problem and the observation succeeded." };
    }

    const gap: CapabilityGap = prior ?? {
      id: genSkillId(),
      problem: observation.problem,
      existingSkillsUsed: [...observation.existingSkillsUsed],
      missingCapability: observation.workaroundSteps.join("; ") || observation.problem,
      occurrences: 0,
      investigationIds: [],
      candidateSkillName: `Investigate: ${observation.problem}`.slice(0, 100),
      candidateSkillCategory: categoryFor(observation.problem),
      detectedAt: Date.now(),
    ...
    // One occurrence is telemetry; repeated failure/workaround is evidence of
    // a reusable capability gap. The threshold prevents skill explosion.
    if (occurrences < 2) {
      return { action: "TRACK_GAP", reason: "Capability gap observed once; wait for recurrence before proposing a new skill.", gap };
    }

    const candidate: SkillCandidate = {
      id: genSkillId(),
      name: gap.candidateSkillName,
      observedInInvestigations: investigations,
      occurrenceCount: occurrences,
      recurringProblem: observation.problem,
      existingWorkaround: [...new Set(observation.workaroundSteps)],
      potentialReuse: occurrences >= 4 ? "HIGH" : "MODERATE",
      proposedCategory: gap.candidateSkillCategory,
      detectedAt: Date.now(),
    };
    ...
    const proposal: SkillProposal = {
      ...
      procedure: makeSteps(observation),
      candidateSubskills: observation.existingSkillsUsed,
      expectedBenefit: "Reduce repeated work while increasing evidence yield and procedural consistency.",
      exampleUseCases: [observation.problem],
      knownRisks: ["Overfitting to one investigation", "Premature activation", "Duplicating an existing skill"],
      validationPlan: "Run sandbox benchmarks against representative prior investigations before activation.",
      createdFromInvestigations: investigations,
      status: "PROPOSED",
      provenance: {
        type: "MODEL_PROPOSED",
        originatingInvestigation: observation.investigationId,
        createdFromCapabilityGap: gap.id,
        createdAt: Date.now(),
      },
      createdAt: Date.now(),
    };
  ...
  /** Find safe compositions from already-active skills. No activation occurs. */
  findComposition(parentSkillId: string, childSkillIds: string[]): SkillLearningDecision {
    ...
    if (!parent || children.length !== childSkillIds.length) {
      return { action: "NO_ACTION", reason: "Composition requires registered parent and child skills." };
    }
    if (children.some(skill => skill.status !== "ACTIVE")) {
      return { action: "NO_ACTION", reason: "Only active validated skills may be proposed for composition." };
    }
    if (parent.subskills.some(id => childSkillIds.includes(id))) {
      return { action: "NO_ACTION", reason: "The requested composition already exists in the parent skill." };
    }
    return {
      action: "PROPOSE_COMPOSITION",
      reason: "Existing validated skills can be composed into a reusable procedure; activation remains gated by validation.",
      composition: { parentSkillId, childSkillIds, reason: "Recurring workflow can be expressed as existing reusable capabilities." },
    };
  }
