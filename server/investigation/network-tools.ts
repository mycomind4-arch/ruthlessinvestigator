...
export async function webFetch(url: string, timeoutMs = 15000): Promise<RetrievedSource> {
  ...
  return {
    url: response.url || safe.toString(), title: response.url || safe.toString(), content,
    contentType, status: response.status, retrievedAt: Date.now(), contentHash: hash(content),
    publisher: safe.hostname, tool: "web_fetch", externalContent: true,
  };
}
...
export async function webSearch(query: string, maxResults = 8): Promise<SearchResult[]> {
  ...
  while ((match = pattern.exec(html)) && results.length < maxResults) {
    ...
    results.push({ title, url, snippet: "", source: "DUCKDUCKGO_HTML" });
  }
  return results;
}
...
export async function githubSearch(query: string, maxResults = 8): Promise<SearchResult[]> {
  ...
  const url = `https://api.github.com/search/repositories?q=${encodeURIComponent(query)}&per_page=${Math.min(maxResults, 30)}`;
  ...
  return (data.items ?? []).map(item => ({ title: item.full_name, url: item.html_url, snippet: item.description ?? "", source: "DUCKDUCKGO_HTML" }));
}
...
export async function executeNetworkTool(request: NetworkRequest): Promise<NetworkResult> {
  ...
  try {
    if (request.tool === "web_search") {
      const results = await webSearch(request.query ?? "", request.maxResults ?? 8);
    ...
    if (request.tool === "github_search") {
      const results = await githubSearch(request.query ?? "", request.maxResults ?? 8);
      return { ok: true, tool: request.tool, query: request.query, sources: results.map(r => ({ url: r.url, title: r.title, content: r.snippet, contentType: "github-repository", status: 200, retrievedAt: Date.now(), contentHash: hash(r.url), tool: request.tool, externalContent: true })), durationMs: Date.now() - started };
    }
